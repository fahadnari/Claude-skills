...
**→ Best used after PRD Generator**
The `prd-generator` skill writes the spec; this skill breaks it. The recommended loop is:

```
PRD Generator → Devil's Advocate → revise PRD → Devil's Advocate (optional second pass)
```

> "Here's the PRD I just wrote. Run devil's advocate on it."

After the challenge, go back to the PRD and address the critical questions raised before your next review. A PRD that has survived Devil's Advocate is significantly stronger than one that hasn't.
