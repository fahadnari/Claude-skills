...
## Output
Save to `specs/outputs/experiment-[name]-[YYYY-MM-DD].md`
...
**For web A/B tests:** Use the `ab-test-designer` skill instead — it includes statistical sample size calculations.

> **Not sure which skill to use?**
> Use **A/B Test Designer** when you can randomly split web/app traffic and need statistical significance.
> Use **Experiment Designer** (this skill) for everything else — pricing changes, feature betas, pilots, fake doors, or any experiment where random assignment isn't possible.
...
After the experiment concludes, save a summary entry to `specs/outputs/experiment-log.md` using this shared format (also used by the `ab-test-designer` skill — keeping both in one log makes it easy to review all experiments together):

```markdown
## [Experiment Name] — [Beta / Pilot / Cohort / Fake Door]

| Field | Value |
|-------|-------|
| **Type** | [Beta / Pilot / Cohort rollout / Fake door] |
| **Owner** | [Name] |
| **Run dates** | [Start] → [End] |
| **Learning goal** | [The question we were trying to answer] |
| **Primary metric** | [Metric]: [baseline] → [result] |
| **Result** | ✅ Confirmed / ❌ Disconfirmed / ⚠️ Mixed / 🔄 Inconclusive |
| **Decision** | [Ship / Kill / Iterate — and why] |
| **Key learnings** | [What we learned, even if it "failed"] |
...
- [ ] Append entry to `specs/outputs/experiment-log.md`
- [ ] Update baseline metrics if behavior changed
- [ ] Share learnings with team — failed experiments teach as much as successful ones
